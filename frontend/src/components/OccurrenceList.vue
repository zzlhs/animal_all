...
defineEmits(['hover-start', 'hover-end', 'select', 'load-more'])
</script>
  ...
  >
    <header class="occurrence-list__header">
      <div>
        ...
        <h2>{{ translate(language, 'list.title', { count: total || records.length }) }}</h2>
    ...
    <footer class="occurrence-list__footer">
      ...
      <span>{{ translate(language, 'list.footer', { loaded: records.length, count: total || records.length }) }}</span>
    </footer>
  </aside>
</template>
