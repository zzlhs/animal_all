...
import { AudioLines, CalendarDays, Database, ExternalLink, MapPin, Ruler, Video } from '@lucide/vue'
  ...
  >
    <div class="occurrence-card__hero" :class="`taxon-${record.class?.toLowerCase() || 'other'}`">
      <ImageGallery
        ...
        :language="language"
        @interaction-start="$emit('hover-start')"
        @interaction-end="$emit('hover-end')"
      />
    </div>
    <div class="occurrence-card__body">
      ...
      <p v-if="record.vernacularName" class="occurrence-card__vernacular">{{ record.vernacularName }}</p>
